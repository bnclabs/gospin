// HTTP client API to access fail-safe dictionary.
//
...
// GetCAS for fail-safe dictionary.
func (c *SafeDictClient) GetCAS() (CAS uint64, err error) {
    CAS, err = c.doHTTP(nil, nil, "HEAD")
    if err != nil {
...
// Set value of the field located by `path` jsonpointer. Set does not wait for
// mutation to be committed to the dictionary.
...
// SetCAS does not wait for the mutation to be committed to the dictionary.
...
// Delete field located by `path` jsonpointer. Delete does not wait for the
// mutation to be committed.
...
// DeleteCAS field located by `path` jsonpointer with matching CAS. DeleteCAS
// does not wait for the mutation to be committed.
...
func (c *SafeDictClient) doHTTP(
    ...
    method string) (CAS uint64, err error) {
